...
import '../entities/git_config_entity.dart';

/// Git仓库抽象接口
abstract class GitRepository {
  /// 初始化Git仓库
  Future<GitRepositoryEntity> initRepository(String repoPath,
      {String? initialBranch});

  /// 检查是否为Git仓库
  Future<bool> isGitRepository(String repoPath);

  /// 获取仓库信息
  Future<GitRepositoryEntity> getRepository(String repoPath);

  /// 获取仓库状态
  Future<GitRepositoryEntity> getRepositoryStatus(String repoPath);

  /// 获取文件状态列表
  Future<List<GitFileStatusEntity>> getFileStatuses(String repoPath);
  ...
  /// 获取单个提交信息
  Future<GitCommitEntity> getCommit(String repoPath, String sha);
  ...
  /// 获取提交差异
  Future<List<GitDiffEntity>> getCommitDiff(String repoPath, String sha);
  ...
  /// 获取配置
  Future<GitConfigEntity> getConfig(String repoPath);

  /// 保存配置
  Future<void> saveConfig(String repoPath, GitConfigEntity config);
  ...
  /// 获取最后一次提交
  Future<GitCommitEntity?> getLastCommit(String repoPath, {String? branch});

  /// 重置到指定提交
  Future<void> resetToCommit(String repoPath, String sha, {bool hard = false});
