...
import 'package:tencent_cloud_chat_demo/common/widgets/background/animated_background.dart';
import 'package:intl/intl.dart';
import 'package:todos_repository/todos_repository.dart';
import 'package:tencent_cloud_chat_demo/core/utils/yxf_utils.dart';
...
class _YxfHomePageState extends State<YxfHomePage> {
  // 被选中的事件
  late ValueNotifier<List<Todo>> _selectedEvents =
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('更新待办失败')),
        );
      }
    }
  }
  ...
  Future<void> _deleteTodo(Todo todo) async {
    try {
            ...
            action: SnackBarAction(
              ...
              onPressed: () async {
                try {
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('撤销失败')),
                    );
                  }
                }
              },
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('删除待办失败')),
        );
      }
    }
  }
  ...
  Future<void> _toggleSubtodosExpanded(Todo todo) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('更新展开状态失败')),
        );
      }
    }
  }
              ...
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    ...
                    final selectedDate = DateTime(_selectedDay.year,
                        _selectedDay.month, _selectedDay.day);
                    final todayDate = DateTime(now.year, now.month, now.day);

                    DateTime? defaultCreatedAt;
                    if (selectedDate.isAfter(todayDate)) {
                      defaultCreatedAt = DateTime(_selectedDay.year,
                          _selectedDay.month, _selectedDay.day, 7, 0);
                    }
