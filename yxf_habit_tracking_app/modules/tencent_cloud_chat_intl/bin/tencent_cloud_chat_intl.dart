...
void main(List<String> arguments) async {
  ...
  while (!isPathValid) {
    ...
    try {
      packagePath ??=
          pubspec['dependency_overrides']['tencent_cloud_chat_intl']['path'];
    } catch (_) {}
    try {
      packagePath ??=
          pubspec['dependencies']['tencent_cloud_chat_intl']['path'];
    } catch (_) {}
        ...
        if (inputPath != null && inputPath.isNotEmpty) {
          ...
          if (!p.isAbsolute(inputPath)) {
            ...
            packagePath = p.normalize(
              p.join(currentWorkingDirectory, inputPath),
            );
          } else {
  ...
  Directory(p.join(packagePath, 'lib', 'language_arb'))
      ...
      .forEach((arbFile) {
        ...
        try {
          ...
          Map<String, dynamic> jsonContent = json.decode(
...
void getOriginalLanguageArb(String packagePath) async {
  ...
  final languageDirectory = Directory(
    p.join(packagePath, "lib", "language_arb"),
  );
...
void addLanguageToBase(String packagePath) async {
  ...
  if (result.exitCode == 0) {
    ...
    stdout.write(
      "Failed to generate localization files. Please manually run 'flutter gen-l10n' in the tencentcloud_chat_uikit_intl package directory.\n",
    );
  }
}
