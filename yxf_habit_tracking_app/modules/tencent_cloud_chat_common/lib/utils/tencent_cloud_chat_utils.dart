...
class TencentCloudChatUtils {
  ...
  // todo: 腾讯SDK,涉及即时通讯模块，包含message、group、plugin等
  /*
    ...
    if (message != null) {
      ...
      if (message.status == 6 && needStatus) {
        if (message.revokerInfo != null &&
            TencentCloudChatUtils.checkString(message.revokerInfo?.userID) !=
                null) {
          text = tL10n.memberRecalledMessage(TencentCloudChatUtils.checkString(
                  message.revokerInfo?.nickName) ??
              message.revokerInfo!.userID!);
        } else {
    ...
    if (tips != null) {
      ...
      switch (type) {
        ...
        case GroupTipsElemType.V2TIM_GROUP_TIPS_TYPE_GROUP_INFO_CHANGE:
          res = opMemberDisplayText +
              groupChangeInfoList
                  .map((e) => buildGroupChangeInfoText(e))
                  .join(",");
          break;
        case GroupTipsElemType.V2TIM_GROUP_TIPS_TYPE_MEMBER_INFO_CHANGE:
          if (memberChangeInfoList.isNotEmpty) {
            V2TimGroupMemberChangeInfo? memberChangeInfo =
                memberChangeInfoList[0];
            int muteTime = memberChangeInfo?.muteTime ?? 0;
            if (muteTime > 0) {
              res = tL10n.banned(membersDisplayText,
                  TencentCloudChatIntl.formatSeconds(muteTime));
            } else {
  ...
  static String getMessageSenderName(V2TimMessage message) {
    return checkString(message.friendRemark) ??
        checkString(message.nickName) ??
        checkString(message.sender) ??
        "";
  }
  ...
  static String buildGroupChangeInfoText(V2TimGroupChangeInfo? info) {
    ...
    switch (type) {
      ...
      case GroupChangeInfoType.V2TIM_GROUP_INFO_CHANGE_TYPE_GROUP_APPROVE_OPT:
        int inviteOptValue =
            info.intValue ?? GroupAddOptType.V2TIM_GROUP_ADD_ANY;
