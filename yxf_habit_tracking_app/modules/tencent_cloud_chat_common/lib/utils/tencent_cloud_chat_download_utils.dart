...
class TencentCloudChatDownloadUtils {
  ...
  static void downloadMessage({
    ...
    //     )
    //     .then((value) {
    //   currentDownloadingList.removeWhere(
    //       (ele) => ele.getUniqueueKey() == "${msgID}_${imageType}_$isSnapshot");
    //   if (value.code == 0) {
  ...
  static bool isInDownloadQueue({
    ...
    if (downloadQueue.containsKey(data.convID)) {
      if (downloadQueue[data.convID] != null) {
        ...
        if (downloadQueue[data.convID]!.indexWhere(
                (ele) => ele.getUniqueueKey() == data.getUniqueueKey()) >
            -1) {
          res = true;
        }
      }
    ...
    console(
        "${data.convID} ${data.getUniqueueKey()} is in download queue: $res");
    return res;
  }
  ...
  static bool isDownloading({
    ...
    bool res = false;
    if (currentDownloadingList.indexWhere(
            (ele) => ele.getUniqueueKey() == data.getUniqueueKey()) >
        -1) {
      res = true;
    }
  ...
  /// 由于使用了tencentCloudChat sdk,将其注释掉
  // static void updateCurrentDownloadMessage(
  //     V2TimMessageDownloadProgress progress) {
  //   String messageIdentfif =
  //       "${progress.msgID}_${progress.type}_${progress.isSnapshot}";

  //   int idx = currentDownloadingList
  //       .indexWhere((ele) => ele.getUniqueueKey() == messageIdentfif);
  //   if (idx > -1) {
  //     DownloadMessageQueueData currentProgressData =
  //         currentDownloadingList[idx];
  ...
  //       console(
  //           "update download progress ${currentDownloadingList[i].currentDownloadSize} ${currentDownloadingList[i].totalSize}");
  //     }
  //     TencentCloudChat.instance.dataInstance.messageData
  //         .updateDownloadingMessage(currentDownloadingList);
  //   }
  // }

  // static void handleDownloadProgressEvent(
  //     V2TimMessageDownloadProgress progress) {
  //   String messageIdentfif =
  //       "${progress.msgID}_${progress.type}_${progress.isSnapshot}";
  //   if (!messageDownloadFinishedList.contains(messageIdentfif) &&
  //       progress.isFinish == true) {
  //     messageDownloadFinishedList.add(messageIdentfif);
  //     console(
  //         "${progress.msgID} isError ${progress.isError} download finished");
  ...
  // static void updateMessageLocalUrlInMemory(
  //     V2TimMessageDownloadProgress progress) {
  //   String messageIdentfif =
  //       "${progress.msgID}_${progress.type}_${progress.isSnapshot}";
  ...
  //       var msgList = TencentCloudChat.instance.dataInstance.messageData
  //           .getMessageList(key: currentProgressData.key);
  ...
  //           messageList: msgList,
  //           userID: currentProgressData.conversationType ==
  //                   ConversationType.V2TIM_C2C
  //               ? currentProgressData.key
  //               : null,
  //           groupID: currentProgressData.conversationType ==
  ...
  static startDownload(Timer timer) {
    ...
    for (var i = 0; i < convIDs.length; i++) {
      ...
      if (downloadQueue.containsKey(convID)) {
        List<DownloadMessageQueueData> convDownloadList =
            downloadQueue[convID] ?? [];
        if (convDownloadList.isEmpty) {
          continue;
        }
        console("current download queue convIDs is ${json.encode(convIDs)}");
        int indexForConvID =
            currentDownloadingList.indexWhere((ele) => ele.convID == convID);
        if (indexForConvID > -1) {
          console(
              "$convID has downloading message. break. the message is ${currentDownloadingList[indexForConvID].path}");
          continue;
        }
  ...
  static void console(String logs) {
    TencentCloudChat.instance.logInstance
        .console(componentName: _tag, logs: logs);
  }
  ...
  static void addDownloadMessageToQueue({
    ...
    if (downloadQueue.containsKey(data.convID)) {
      if (downloadQueue[data.convID] != null) {
        int idx = downloadQueue[data.convID]!.indexWhere(
            (element) => element.getUniqueueKey() == data.getUniqueueKey());
        if (idx > -1) {
          console(
              "${data.getUniqueueKey()} is already add to the download queue");
          return;
        }
      }
    } else {
    ...
    int curDownloadingIdx = currentDownloadingList
        .indexWhere((ele) => ele.getUniqueueKey() == data.getUniqueueKey());
    ...
    console(
        "add ${data.msgID} to the download queue. message type ${data.messageType}");
  }
  ...
  static void removeFromDownloadQueue({
    ...
    if (downloadQueue.containsKey(data.convID)) {
      if (downloadQueue[data.convID] != null) {
        downloadQueue[data.convID]!.removeWhere(
            (ele) => ele.getUniqueueKey() == data.getUniqueueKey());
        console("remove ${data.getUniqueueKey()} from download queue success");
      }
    } else {
      console(
          "the ${data.getUniqueueKey()} is not add to download. pealse check");
    }
  }
}
