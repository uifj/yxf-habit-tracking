...
///
/// * 主题管理器 - 单例模式设计
// - 核心功能 ：统一管理整个应用的主题数据，支持亮色/暗色模式切换
// - 设计模式 ：采用单例模式确保全局唯一性
// - 事件驱动 ：通过 EventBus 实现主题变更的实时通知
// - 关键方法 ：
//   - init() : 初始化主题配置
//   - toggleBrightnessMode() : 切换亮暗模式
//   - setThemeColors() : 动态设置主题颜色
//   - getThemeData() : 生成 Flutter ThemeData
class TencentCloudChatTheme {
  ...
  factory TencentCloudChatTheme(
      {TencentCloudChatThemeModel? themeMode, Brightness? brightness}) {
  ...
  static init(
      {Brightness? brightness, TencentCloudChatThemeModel? themeModel}) {
    if (brightness != null) {
  ...
  set themeModel(TencentCloudChatThemeModel value) {
    ...
    TencentCloudChat.instance.eventBusInstance
        .fire(this, "TencentCloudChatTheme");
  }
