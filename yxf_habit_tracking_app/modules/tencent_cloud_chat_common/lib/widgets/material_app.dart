...
/// * 是腾讯云聊天 UIKit 的核心应用容器，它是对 Flutter 原生 MaterialApp 的高级封装。
/// 主要特性：
// 1. 主题系统集成
// - 自动集成腾讯云聊天主题系统，支持明暗主题切换
// - 通过 EventBus 监听主题变化，实现动态主题更新
// - 提供默认的明暗主题配置，可通过 theme.getThemeData() 生成 Flutter ThemeData
// 2. 国际化支持
// - 集成 TencentCloudChatLocalizations 本地化系统
// - 支持缓存用户语言偏好设置
// - 异步初始化本地化配置，确保应用启动时正确加载语言资源
// 3. 导航观察器
// - 自动注册腾讯云聊天的导航观察器 TencentCloudChat.instance.navigatorObserver
// - 用于跟踪页面导航状态，支持聊天功能的生命周期管理
// 4. 缓存系统初始化
// - 在应用启动时自动初始化缓存系统
// - 支持持久化存储用户偏好设置和聊天数据
...
class _TencentCloudChatMaterialAppState
  ...
  Stream<TencentCloudChatTheme>? themeDataListener = TencentCloudChat
        ...
        builder: (context, snapshot) {
          if (isInitIntl || snapshot.data != null) {
            ...
            if (widget.themeMode != null) {
              theme.brightnessWithoutFire = themeMode == ThemeMode.light
                  ? Brightness.light
                  : Brightness.dark;
            }
