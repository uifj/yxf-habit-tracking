...
import 'package:flutter/scheduler.dart'; // * 调度器，用于确保帧率运行
...
import 'package:tencent_cloud_chat_common/eventbus/tencent_cloud_chat_eventbus.dart'; // * 事件总线，用于组件之间的通信
...
/// * 抽象的状态管理基类，为组件提供统一的状态管理和跨平台适配功能.
/// 这个state对象注册成为了一个事件总线的监听器，当事件总线触发事件时，会调用[onEvent]方法.
abstract class TencentCloudChatState<T extends StatefulWidget> extends State<T>
    with WidgetsBindingObserver {
  // EventBus instance，创建了事件总线的实例
  TencentCloudChatEventBus eventbus =
