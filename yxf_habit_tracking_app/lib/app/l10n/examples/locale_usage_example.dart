...
/// Example showing how to use the new locale management system
/// 展示如何使用新语言管理系统的示例
class LocaleUsageExample extends StatelessWidget {
  const LocaleUsageExample({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const _LocaleExampleApp(),
    );
  }
}

class _LocaleExampleApp extends StatelessWidget {
  const _LocaleExampleApp();
...
/// Example of using locale in a specific widget
/// 在特定组件中使用语言的示例
      ...
      builder: (context, state) {
        return Column(
          children: [
            Text(
              context.isEnglish
                  ? 'Current language: ${state.languageName}'
                  : '当前语言: ${state.languageName}',
            ),
            Text(
              context.l10n.todosOverviewAppBarTitle,
              style: Theme.of(context).textTheme.headlineMedium,
            ),
            ElevatedButton(
              onPressed: () => context.toggleLanguage(),
              child: Text(
                context.isEnglish ? 'Switch Language' : '切换语言',
              ),
            ),
          ],
        );
